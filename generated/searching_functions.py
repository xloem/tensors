def argmax(x, /, *, axis=None, keepdims=False):
    ...
    the maximum value occurs multiple times, only the indices
    ...
    *   **axis**: *Optional[ int ]*
        ...
        *   axis along which to search. If ``None``, the function must
            return the index of the maximum value of the flattened array.
            Default: ``None``.
    ...
    *   **out**: *<array>*
        ...
        *   if ``axis`` is ``None``, a zero-dimensional array containing
...
def argmin(x, /, *, axis=None, keepdims=False):
    ...
    the minimum value occurs multiple times, only the indices
    corresponding to the first occurrence are returned.
    Parameters
    **********
    ...
    *   **axis**: *Optional[ int ]*
        ...
        *   axis along which to search. If ``None``, the function must
            return the index of the minimum value of the flattened array.
            Default: ``None``.
...
def nonzero(x, /):
    ...
    *   **out**: *Tuple[ <array>, … ]*
        ...
        *   a tuple of ``k`` arrays, one for each dimension of ``x`` and
            each of size ``n`` (where ``n`` is the total number of
            non-zero elements), containing the indices of the non-zero
...
def where(condition, x1, x2, /):
    ...
    *   **condition**: *<array>*
        ...
        *   when ``True``, yield ``x1_i``; otherwise, yield ``x2_i``. Must
            be compatible with ``x1`` and ``x2`` (see `Broadcasting
            <#broadcasting>`_).
    
    *   **x1**: *<array>*
    
        *   first input array. Must be compatible with ``condition`` and
            ``x2`` (see `Broadcasting <#broadcasting>`_).
    
    *   **x2**: *<array>*
    
        *   second input array. Must be compatible with ``condition`` and
            ``x1`` (see `Broadcasting <#broadcasting>`_).
    Returns
    *******
    
    *   **out**: *<array>*
    
        *   an array with elements from ``x1`` where ``condition`` is
            ``True``, and elements from ``x2`` elsewhere. The returned
